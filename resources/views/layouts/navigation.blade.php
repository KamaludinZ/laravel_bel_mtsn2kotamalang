<nav x-data="{ open: false }" class="bg-white dark:bg-gray-800 border-b border-gray-100 dark:border-gray-700">
            ...
            <div class="flex">
                ...
                <div class="shrink-0 flex items-center">
                    <a href="{{ route('dashboard') }}" class="flex items-center gap-3">
                        ...
                        @endphp
                        @if($appLogo)
                            <img src="{{ asset('storage/' . $appLogo) }}" alt="Logo" class="h-9 w-auto">
                        @else
                            <x-application-logo class="block h-9 w-auto fill-current text-gray-800 dark:text-gray-200" />
                        @endif
                        <span class="text-lg font-semibold text-gray-800 dark:text-gray-200 hidden md:block">{{ $appName }}</span>
                    </a>
                </div>
                ...
                <div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex">
                    ...
                    <x-nav-link :href="route('audio-libraries.index')" :active="request()->routeIs('audio-libraries.*')">
                        {{ __('Kelola Audio') }}
                    </x-nav-link>
                    <x-nav-link :href="route('bell-schedules.index')" :active="request()->routeIs('bell-schedules.*')">
                        {{ __('Kelola Jadwal') }}
                    </x-nav-link>
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        <div class="pt-2 pb-3 space-y-1">
            ...
            <x-responsive-nav-link :href="route('audio-libraries.index')" :active="request()->routeIs('audio-libraries.*')">
                {{ __('Kelola Audio') }}
            </x-responsive-nav-link>
            <x-responsive-nav-link :href="route('bell-schedules.index')" :active="request()->routeIs('bell-schedules.*')">
                {{ __('Kelola Jadwal') }}
            </x-responsive-nav-link>
