        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    <form action="{{ route('bell-schedules.store') }}" method="POST">
                        @csrf
                        ...
                        <div class="mb-6">
                            ...
                            <select name="bell_type_id" id="bell_type_id" required
                                ...
                                @foreach($bellTypes as $bellType)
                                    <option value="{{ $bellType->id }}" {{ old('bell_type_id') == $bellType->id ? 'selected' : '' }}>
                                        {{ $bellType->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="mb-6">
                            ...
                            <select name="audio_library_id" id="audio_library_id" required
                                ...
                                @foreach($audioLibraries as $audio)
                                    <option value="{{ $audio->id }}" {{ old('audio_library_id') == $audio->id ? 'selected' : '' }}>
                                        {{ $audio->title }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="mb-6">
                            ...
                            <textarea name="keterangan" id="keterangan" rows="3"
                                ...
                                placeholder="Tambahkan keterangan jadwal (opsional)...">{{ old('keterangan') }}</textarea>
                            @error('keterangan')
                                <p class="mt-2 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                            @enderror
                        </div>
